...
function findActiveMention(
  ...
  const safeOffset = Math.max(
    0,
    Math.min(cursorOffset, text.length)
  );
  ...
  const previousCharacter =
    token[mentionStart - 1];

  if (
    previousCharacter &&
    isMentionQueryCharacter(previousCharacter)
  ) {
    return null;
  }
  ...
  if (
    relativeCursor < mentionStart ||
    relativeCursor > mentionEnd
  ) {
    return null;
  }
  ...
  try {
    ...
    const visit = async (
      ...
      for (const entry of dirEntries) {
        ...
        if (
          entry.isDirectory() &&
          RECURSIVE_MENTION_IGNORED_DIRECTORIES.has(
            entry.name
          )
        ) {
          continue;
...
export function InputBar({
  ...
  const activeMentionRef =
    useRef<MentionMatch | null>(null);

  const mentionScrollerRef =
    useRef<ScrollBoxRenderable>(null);
    ...
    useCallback(
      (
        ...
        const mentionChanged =
          previousMention?.start !==
            nextMention.start ||
          previousMention?.end !==
            nextMention.end ||
          previousMention?.query !==
            nextMention.query;
        ...
        if (mentionChanged) {
          ...
          mentionScrollerRef.current?.scrollTo(
            0
          );
        }
      },
  ...
  useKeyboard((key) => {
     ...
     * ENTER
     *
     * O Enter é tratado aqui em vez de depender
     * do onSubmit interno do TextareaRenderable.
     */
    if (
      key.name === "return" ||
      key.name === "enter"
      ...
      /*
       * Se o menu de comandos estiver aberto,
      ...
      /*
       * Se o menu de mentions estiver aberto,
       * Enter seleciona o arquivo.
       */
      if (
        showMentionMenu &&
        mentionCandidates[
      ...
      /*
       * Pega diretamente o conteúdo do textarea.
       *
       * Isso é importante porque inputValue pode
       * estar um render atrás do valor real.
       */
      const textarea =
        textareaRef.current;
     ...
     * CTRL + J
     *
     * Insere uma nova linha no local
     * atual do cursor.
     */
    if (
      key.ctrl &&
